...
public function getMacAddress()
{
    ...
    if (PHP_OS_FAMILY === 'Windows') {
        ...
        $matches = [];
        preg_match('/Physical Address[^:]*:\s*([^\s]+)/', $output, $matches);
        if (isset($matches[1])) {
        ...
        $matches = [];
        preg_match('/\bether\s+([^\s]+)/i', $output, $matches);
        if (isset($matches[1])) {
            $macAddress = $matches[1];
        }
    } else {
        // Système d'exploitation non pris en charge
        return "Impossible de récupérer l'adresse MAC sur ce système d'exploitation.";
    }

    if ($macAddress) {
        return "Adresse MAC de l'ordinateur portable : " . $macAddress;
    } else {
        return "Adresse MAC non trouvée pour l'ordinateur portable.";
    }
}
